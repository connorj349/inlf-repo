...
@export var loot_table_item_datas: Array[ItemData] # change to loot_table
...
func _on_Timer_timeout():
	if does_reset and loot_table_item_datas.size() > 0:
		...
		var random_amount = randi() % loot_table_item_datas.size()
		for _i in range(0, random_amount):
			var new_slot = SlotData.new()
			var random_item = loot_table_item_datas[randi() % loot_table_item_datas.size()] # choose a random item
