...
func on_inventory_interact(inventory_data, index, button): #when any inventory ui is interacted with
	match [grabbed_slot_data, button]:
		[null, BUTTON_LEFT]: #currently no slot data grabbed and left clicking
			grabbed_slot_data = inventory_data.grab_slot_data(index)
		[_, BUTTON_LEFT]: #anything(wildcard) info, and left click
			grabbed_slot_data = inventory_data.drop_slot_data(grabbed_slot_data, index)
		[null, BUTTON_RIGHT]: #case for using item
			inventory_data.use_slot_data(index)
		[_, BUTTON_RIGHT]: #dropping item
